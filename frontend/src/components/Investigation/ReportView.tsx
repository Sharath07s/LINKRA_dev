...
const SOURCE_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  structural_analysis: { label: "STRUCTURAL ANALYSIS", cls: "bg-blue-900/40 text-blue-400 border-blue-700/50" },
...
function Section({ icon: Icon, title, count, sourceType, children, defaultOpen = false }:
  ...
  return (
    <div className="border border-slate-800 rounded-xl overflow-hidden">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-3 p-4 bg-slate-900/60 hover:bg-slate-900 transition-colors text-left"
      >
        <Icon className="h-4 w-4 text-slate-400 shrink-0" />
        <span className="font-semibold text-sm text-slate-200 flex-1">{title}</span>
        {count !== undefined && (
          <span className="text-xs font-mono text-slate-500 mr-2">{count}</span>
        )}
        <SourceBadge type={sourceType} />
        {open ? <ChevronDown className="h-3 w-3 text-slate-500 ml-2" /> : <ChevronRight className="h-3 w-3 text-slate-500 ml-2" />}
      </button>
      {open && <div className="p-4 bg-slate-950/60 space-y-2">{children}</div>}
    </div>
  );
}

function EmptyState({ message }: { message: string }) {
  return <p className="text-xs text-slate-500 italic">{message}</p>;
}
...
export function ReportView({ investigationId }: ReportViewProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[400px] gap-4 text-slate-400">
        ...
        <div className="text-center">
          <p className="font-semibold text-slate-300 mb-1">Investigation Report</p>
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-[300px] text-slate-500 text-sm animate-pulse">
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 justify-between flex-wrap">
        ...
        <div className="flex gap-2">
          <button
            onClick={loadReport}
            className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-semibold text-slate-300 rounded-lg transition-colors"
          >
            Refresh
          </button>
          <button
          ...
          <button
            onClick={exportJSON}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-semibold text-slate-300 rounded-lg transition-colors"
          >
      ...
      <div className="space-y-2">
        ...
        <Section icon={FileText} title="Investigation Overview" sourceType="observed" defaultOpen>
          ...
          {report.investigation.summary && (
            <p className="text-xs text-slate-400 mt-2 border-t border-slate-800 pt-2">{report.investigation.summary}</p>
          )}
        </Section>
        ...
        <Section icon={Users} title="Verified Entities" count={report.entities.length} sourceType="observed">
          {report.entities.length === 0
            ...
            : report.entities.map((e: any) => (
              <div key={e.entity_id} className="bg-slate-900/40 rounded-lg p-2 text-xs border border-slate-800">
                ...
                {e.aliases?.length > 0 && (
                  <p className="text-slate-500 mt-0.5">Aliases: {e.aliases.join(", ")}</p>
                )}
              </div>
            ))
          }
        ...
        <Section icon={Link2} title="Observed Relationships" count={report.relationships.length} sourceType="observed">
          {report.relationships.length === 0
            ...
            : report.relationships.map((r: any, i: number) => (
              <div key={i} className="text-xs border border-slate-800 rounded-lg p-2 bg-slate-900/40">
                <p className="text-slate-200">
                  <span className="font-semibold">{r.source_entity_name}</span>
                  <span className="mx-1.5 text-blue-400">—{r.relationship_type}→</span>
          ...
          {report.evidence.length === 0
            ...
            : report.evidence.slice(0, 20).map((ev: any, i: number) => (
              <div key={i} className="text-xs border-l-2 border-emerald-700 pl-2 py-0.5">
                <p className="text-slate-300 font-medium">{ev.title}</p>
                <p className="text-slate-500">{ev.description}</p>
                {ev.provenance && (
                  <p className="text-slate-600">{ev.provenance.file_name} — {ev.provenance.source_type}</p>
                )}
              </div>
            ))
          }
          {report.evidence.length > 20 && (
            <p className="text-xs text-slate-500 italic">+{report.evidence.length - 20} more evidence records</p>
          )}
        </Section>
        ...
        <Section icon={TrendingUp} title="Graph Structure (Structural Analytics)" count={report.graph_summary.entity_summaries.length} sourceType="structural_analysis">
          {report.graph_summary.entity_summaries.length === 0
            ...
            : report.graph_summary.entity_summaries.map((gs: any) => (
              <div key={gs.entity_id} className="text-xs border border-slate-800 rounded-lg p-2 bg-slate-900/40">
                <p className="font-semibold text-slate-200">{gs.entity_name}</p>
                <p className="text-slate-500">Degree: {gs.degree} (In: {gs.in_degree}, Out: {gs.out_degree})</p>
              </div>
            ))
          }
        </Section>
        ...
        <Section icon={AlertTriangle} title="Structural Anomalies" count={report.anomalies.length} sourceType="structural_analysis">
          {report.anomalies.length === 0
            ...
            : report.anomalies.map((a: any) => (
              <div key={a.anomaly_id} className="text-xs border border-slate-800 rounded-lg p-3 bg-slate-900/40">
                <div className="flex items-center gap-2 mb-1">
                  <AlertTriangle className="h-3 w-3 text-amber-400" />
                  <span className="font-semibold text-slate-200">{a.entity_name}</span>
                ...
                <p className="text-slate-600 text-[9px] mt-1 italic">{a.disclaimer}</p>
              </div>
            ))
          }
        </Section>
            ...
            : report.potential_links.map((pl: any, i: number) => (
              <div key={i} className="text-xs border border-amber-900/40 rounded-lg p-3 bg-amber-950/20">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-semibold text-slate-200">{pl.source_entity_name}</span>
                  <span className="text-slate-500 mx-1">↔</span>
                  <span className="font-semibold text-slate-200">{pl.target_entity_name}</span>
                  <span className="ml-auto font-mono text-amber-400">{pl.score.toFixed(2)}</span>
                </div>
                <p className="text-slate-400">{pl.reason}</p>
        ...
        {report.ai_section && (
          <Section icon={Sparkles} title="AI-Assisted Assessment" sourceType="ai_generated" defaultOpen>
            ...
            <p className="text-[9px] text-slate-600 mt-2">Provider: {report.ai_section.provider}</p>
          </Section>
        )}
        ...
        {report.limitations?.length > 0 && (
          <div className="border border-slate-800 rounded-xl p-4 bg-slate-900/40">
            <p className="text-xs font-bold text-slate-400 uppercase mb-2">Limitations & Missing Data</p>
