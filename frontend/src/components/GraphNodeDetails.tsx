  ...
  if (!selectedNode) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center text-[#666666] space-y-2">
        <Network className="h-8 w-8 text-[#666666]" />
        <p className="text-xs font-semibold">Focus a Node Link</p>
        <p className="text-[10px] text-[#666666] max-w-[180px]">Select any icon node in the network constellation to view full relationship intelligence</p>
      </div>
    );
  }
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <div className="bg-[#050505]/50 p-2.5 rounded-lg border border-slate-850 text-center">
          <span className="text-[9px] text-[#666666] block uppercase font-bold">Risk Assessment index</span>
          <span className="text-lg font-extrabold text-red-400">{selectedNode.rating} / 10</span>
        </div>

        <div className="space-y-1 text-xs">
          <span className="text-[9px] font-bold text-[#666666] uppercase block">Profile Summary</span>
          <p className="text-[#F5F5F5] leading-normal bg-[#050505]/40 p-3 rounded-lg border border-slate-850">
            {selectedNode.desc}
          </p>
        </div>

        {/* M1.8 Degree Analytics */}
        <div className="bg-[#080808]/50 p-2.5 rounded-lg border border-[#222222]">
          <span className="text-[9px] text-[#9A9A9A] block uppercase font-bold mb-2 flex items-center gap-1">
            <Activity className="h-3 w-3" /> Connectivity Metrics (M1.8)
          </span>
          {loadingAnalytics ? (
             <div className="text-[10px] text-[#666666] animate-pulse">Calculating network metrics...</div>
          ) : (
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-[#050505] p-1.5 rounded border border-[#222222]">
                <span className="text-[8px] text-[#666666] uppercase block">Total</span>
                <span className="text-xs font-bold text-white">{degree?.total_degree || 0}</span>
              </div>
              <div className="bg-[#050505] p-1.5 rounded border border-[#222222]">
                <span className="text-[8px] text-[#666666] uppercase block">In</span>
                <span className="text-xs font-bold text-[#10B981]">{degree?.in_degree || 0}</span>
              </div>
              <div className="bg-[#050505] p-1.5 rounded border border-[#222222]">
                <span className="text-[8px] text-[#666666] uppercase block">Out</span>
        ...
        {!loadingAnalytics && distribution.length > 0 && (
          <div className="space-y-1">
            <span className="text-[9px] font-bold text-[#666666] uppercase block">Relationship Types</span>
            <div className="flex flex-wrap gap-1.5 mt-1">
              {distribution.map((dist: any, idx: number) => (
                <span key={idx} className="px-1.5 py-0.5 rounded text-[8px] font-mono border border-[#2A2A2A] bg-[#0D0D0D] text-[#F5F5F5]">
        ...
        <div className="bg-[#080808]/50 p-2.5 rounded-lg border border-[#222222] relative overflow-hidden">
          <span className="text-[9px] text-[#9A9A9A] block uppercase font-bold mb-2 flex items-center gap-1">
            <AlertTriangle className="h-3 w-3 text-red-500" /> Anomaly Signals (M1.9)
          </span>
          {loadingAnalytics ? (
            <div className="text-[10px] text-[#666666] animate-pulse">Analyzing structure...</div>
          ) : anomalies.length > 0 ? (
            <div className="space-y-2 relative z-10">
              {anomalies.map((anom: any, idx: number) => (
                <div key={idx} className="bg-[#050505] border-l-2 border-red-500 p-2 rounded relative">
                  ...
                  <p className="text-[9px] text-[#9A9A9A] mt-1 leading-tight">{anom.reason}</p>
                  
                  {anom.explanation && (
                    <div className="mt-2 bg-[#080808]/50 border border-[#222222] p-2 rounded">
                      <span className="text-[8px] uppercase font-bold text-[#666666] mb-1 block">Explanation</span>
                      <p className="text-[9px] text-[#F5F5F5] italic">{anom.explanation.reason}</p>
                    </div>
                  )}

                  <div className="flex justify-between text-[8px] text-[#666666] mt-1.5 border-t border-[#222222] pt-1">
                    <span>Observed: <span className="text-[#F5F5F5] font-mono">{anom.observed_value}</span></span>
                    <span>Baseline: <span className="text-[#F5F5F5] font-mono">{anom.baseline_value}</span></span>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-[10px] text-[#666666] italic py-1 text-center bg-[#050505]/40 rounded border border-[#222222]/50">
        ...
        <div className="space-y-2 flex-1">
          <span className="text-[9px] font-bold text-[#9A9A9A] uppercase block tracking-wider">Direct Links ({directEdges.length})</span>
            ...
            {directEdges?.map((edge: any, idx: number) => {
              ...
              return (
                <div 
                  ...
                  className="p-2.5 bg-[#050505]/60 hover:bg-[#080808] border border-slate-850 rounded-xl flex items-center justify-between text-[10px] cursor-pointer transition-colors"
                >
                  <div className="flex flex-col min-w-0">
                    <span className="font-semibold text-[#F5F5F5] truncate">{targetNode?.label}</span>
                    <span className="text-[8px] text-[#666666] mt-0.5">{edge.relation}</span>
                  </div>
                  <span className="text-[9px] font-bold text-[#10B981] font-mono">{edge.weight}%</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-[#080808]/50 p-2.5 rounded-lg border border-[#222222] relative overflow-hidden">
          ...
          {loadingAnalytics ? (
            <div className="text-[10px] text-[#666666] animate-pulse">Calculating structure...</div>
          ) : potentialLinks.length > 0 ? (
            <div className="space-y-2 relative z-10 max-h-[160px] overflow-y-auto pr-1">
              <div className="text-[8px] text-[#9A9A9A] mb-2 italic">Based strictly on shared graph topology. Not confirmed criminal activity.</div>
              {potentialLinks.map((link: any, idx: number) => {
                ...
                return (
                  <div key={idx} 
                    ...
                    className="bg-[#050505] hover:bg-[#080808] border-l-2 border-fuchsia-500 p-2 rounded relative cursor-pointer transition-colors border-y border-r border-y-slate-800 border-r-slate-800">
                    <div className="flex justify-between items-start">
                    ...
                    {link.explanation && (
                      <div className="mt-2 text-[9px] bg-[#080808]/50 p-1.5 rounded border border-[#222222]">
                        <span className="text-[#9A9A9A] font-bold block mb-1 uppercase text-[8px]">Structural Evidence</span>
                        <p className="text-[#F5F5F5] italic leading-tight">{link.explanation.reason}</p>
                      </div>
                    )}
                  </div>
                );
        ...
        <div className="bg-[#080808]/50 p-2.5 rounded-lg border border-[#222222] relative overflow-hidden">
          ...
          {loadingAnalytics ? (
            <div className="text-[10px] text-[#666666] animate-pulse">Retrieving provenance...</div>
          ) : evidence.length > 0 ? (
            <div className="space-y-2 relative z-10 max-h-[160px] overflow-y-auto pr-1">
              {evidence.map((ev: any, idx: number) => (
                <div key={idx} className="bg-[#050505] border-l-2 border-emerald-500 p-2 rounded relative">
                  <div className="flex justify-between items-start">
                  ...
                  {ev.description && (
                    <p className="text-[9px] text-[#9A9A9A] mt-1 leading-tight">{ev.description}</p>
                  )}
                  {ev.provenance && (
                    <div className="mt-1.5 bg-[#080808]/50 p-1.5 rounded border border-[#222222] text-[8px]">
                      <span className="text-[#666666] font-bold block mb-0.5 uppercase">Provenance Data</span>
                      <div className="grid grid-cols-2 gap-1 text-[#F5F5F5] font-mono">
        ...
        <div className="p-3 bg-blue-950/15 border border-blue-900/30 rounded-xl space-y-1.5 mt-auto">
          <span className="text-[9px] font-bold text-[#10B981] uppercase flex items-center gap-1">
            <Info className="h-3.5 w-3.5" />
            <span>Security clearance statement</span>
          </span>
          <p className="text-[10px] text-[#9A9A9A] leading-normal">
        ...
        <button 
          ...
          className="w-full py-2.5 bg-[#050505] hover:bg-slate-850 border border-[#222222] hover:border-[#2A2A2A] rounded-xl text-xs font-bold text-[#F5F5F5] hover:text-white transition-all text-center"
        >
