  ...
  return (
    <div className="bg-[#080808]/40 border border-[#222222] rounded-2xl h-full flex flex-col overflow-hidden relative">
      ...
      <div className="p-5 flex-1 overflow-y-auto space-y-6">
        <div>
          <span className="text-[10px] font-bold text-[#666666] uppercase tracking-widest block mb-2 flex items-center gap-1.5">
            <FileText className="h-3 w-3" /> Incident Summary
          </span>
          <p className="text-sm text-[#F5F5F5] leading-relaxed font-serif tracking-wide">
            {data.summary}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[#050505]/60 border border-[#222222] p-4 rounded-xl">
            <span className="text-[10px] font-bold text-[#666666] uppercase tracking-widest block mb-2 flex items-center gap-1.5">
              <Target className="h-3 w-3 text-[#10B981]" /> Modus Operandi & Pattern
            </span>
            <p className="text-xs text-[#F5F5F5] leading-relaxed">
              <span className="text-[#F5F5F5] font-semibold">{data.pattern}</span><br />
              {data.modus_operandi}
            </p>
          </div>
          <div className="bg-[#050505]/60 border border-[#222222] p-4 rounded-xl">
        ...
        <div>
          <span className="text-[10px] font-bold text-[#666666] uppercase tracking-widest block mb-2">Recommended Actions</span>
          <div className="space-y-2">
            {data.recommended_actions?.map((action: string, idx: number) => (
              <div key={idx} className="flex items-center gap-2 p-2.5 bg-blue-950/10 border border-blue-900/20 rounded-lg">
                ...
                <span className="text-xs font-medium text-[#F5F5F5]">{action}</span>
              </div>
            ))}
          </div>
        </div>
