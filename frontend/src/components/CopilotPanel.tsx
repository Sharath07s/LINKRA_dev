...
export function CopilotPanel({ investigationId, entityId }: { investigationId?: string, entityId?: string }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#080808]/60 border border-[#222222] rounded-2xl overflow-hidden">
      {/* Header */}
      <div className="p-3 border-b border-[#222222] bg-[#050505] flex items-center gap-2">
        <Bot className="h-4 w-4 text-[#10B981]" />
        <span className="font-bold text-xs uppercase text-[#F5F5F5] tracking-wider">AI Copilot</span>
        {entityId && <span className="ml-auto text-[9px] bg-[#0D0D0D] px-1.5 py-0.5 rounded text-[#9A9A9A]">Context: Entity</span>}
      </div>

      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-[#666666] space-y-2 opacity-60">
          ...
          messages.map((msg) => (
            <div key={msg.id} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
              <div className={`max-w-[85%] rounded-xl p-3 text-xs ${
                msg.role === "user" 
                  ? "bg-[#10B981]/15 border border-[#10B981]/30 text-[#F5F5F5]" 
                  : "bg-[#050505] border border-[#222222] text-[#F5F5F5]"
              }`}>
                {msg.role === "assistant" ? (
              ...
              {msg.role === "assistant" && msg.evidence && msg.evidence.length > 0 && (
                <div className="mt-1.5 max-w-[85%] flex flex-wrap gap-1">
                  ...
                  {msg.evidence.length > 3 && (
                    <span className="text-[8px] text-[#666666] flex items-center px-1">+{msg.evidence.length - 3} more</span>
                  )}
                </div>
              )}
              ...
              {msg.role === "assistant" && msg.status !== "PROVIDER_UNAVAILABLE" && msg.status !== "INSUFFICIENT_DATA" && msg.status !== "ERROR" && (
                <div className="mt-1 flex items-center gap-1 text-[8px] text-[#666666] italic max-w-[85%]">
        ...
        {loading && (
          <div className="flex items-start">
            <div className="bg-[#050505] border border-[#222222] text-[#666666] rounded-xl p-3 text-xs flex items-center gap-2">
      ...
      <div className="p-3 bg-[#050505] border-t border-[#222222]">
        <div className="relative flex items-center">
          <input
            type="text"
            className="w-full bg-[#080808] border border-[#2A2A2A] rounded-lg pl-3 pr-10 py-2 text-xs text-[#F5F5F5] placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-[#10B981]"
          ...
          <button 
            ...
            className="absolute right-2 p-1.5 text-[#10B981] hover:text-[#10B981] disabled:opacity-50 disabled:hover:text-[#10B981]"
          >
